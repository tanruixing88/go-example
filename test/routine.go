 ...
 *
 * 新加入G的情况, go func(){}()
 * 1.若本地队列不满，如果G是go新创建，或者目前的G是Gwaiter状态（time sleep，channel等触发），
 *   或者由于时间片（sysmon发现某个P执行某个G太久, 函数调用且栈内存大于128B）到达后，会放到队列后面
 * 2.若本地队列满，则会调度一半的G到全局队列
 *
 * 完成G的情况
 * 1.为了保证调度的公平性，每个工作线程每进行61次调度就需要优先从全局运行队列中获取goroutine出来运行
 * 2.若本地队列不空，则继续调度
 * 3.若本地队列为空，全局队列不空则从全局队列取当前P一半大小的G（或者全局队列G总数除P的数量的最小值）调度过来执行
 * 4.若本地队列为空，全局队列为空则会从其他P取一半的G,
 *
 * 网络调用处理
 * 1.若因为非阻塞系统调用则sysmon协程(20us-10ms)会调度解开P和M的关系，P继续和下一个空闲M（若没有则新建一个M）进行绑定并继续执行调度；
 * 将M和G关联进入内核态等待系统回调，如果回调完成M归入空闲队列等待新P继续绑定。若找不到P，则将G放到全局队列中。M则放到线程池
 *
 */
