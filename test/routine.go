...
/**
 * 1. goroutine 本地队列大小256，
 *
 * 新加入G的情况, go func(){}()
 * 若本地队列满，且其他P队列不满为空会调度一半的G到空P中。
 ...
 * 完成G的情况
 * 若本地队列不满，则继续调度
 * 若本地队列为空，则会从其他P取一半的G或者从全局队列取当前P一半大小的G调度过来执行
 *
 * 若因为系统调用则sysmon协程会调度解开P和M的关系，P继续和下一个空闲M（若没有则新建一个M）进行绑定并继续执行调度；将M和G关联进入内核态等待系统回调，如果回调完成M归入空闲队列等待新P继续绑定。
 *
 */
